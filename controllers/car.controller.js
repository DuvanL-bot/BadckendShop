...
async function saveInfoCart(id_product, id_user, quantity, created_at
  ...
  const query =
   "INSERT INTO cart (id_product, id_user,quantity, created_at) VALUES($1, $2, $3, $4) RETURNING *"
...
async function getInfoCart(user_id) {
  const query =
    "SELECT p.id_product, p.title_product, p.description_product, p.price_product, p.thumbnail_product, d.category_details FROM cart c INNER JOIN products p ON c.id_product = p.id_product INNER JOIN details d ON p.id_product= d.id_product WHERE c.id_user= $1"
